...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    public void onCreate() { checkForFontAndColors(); }
    ...
    public void onTitleChanged(@NonNull TerminalSession updatedSession) {
        ...
        termuxSessionListNotifyUpdated();
        if (mActivity != null) { mActivity.runOnUiThread(() -> mActivity.renderTabBar()); }
    }
    ...
    public void onPasteTextFromClipboard(@Nullable TerminalSession session) {
        ...
        if (text != null) mActivity.getTerminalView().mEmulator.paste(text);
    }
    ...
    public void onBell(@NonNull TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case TermuxPropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell(); break;
            case TermuxPropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                loadBellSoundPool();
                if (mBellSoundPool != null) mBellSoundPool.play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
        }
    }
    ...
    public void onColorsChanged(@NonNull TerminalSession changedSession) {
        if (mActivity.getCurrentSession() == changedSession) updateBackgroundColor();
    }
    ...
    private synchronized void releaseBellSoundPool() {
        if (mBellSoundPool != null) { mBellSoundPool.release(); mBellSoundPool = null; }
    }
    ...
    public void switchToSession(boolean forward) {
        ...
        if (forward) { if (++index >= size) index = 0; } else { if (--index < 0) index = size - 1; }
        TermuxSession termuxSession = service.getTermuxSession(index);
        if (termuxSession != null) setCurrentSession(termuxSession.getTerminalSession());
    }
    ...
    public void renameSession(final TerminalSession sessionToRename) {
        ...
        TextInputDialogUtils.textInput(mActivity, R.string.title_rename_session, sessionToRename.mSessionName, R.string.action_rename_session_confirm, text -> {
            renameSession(sessionToRename, text); termuxSessionListNotifyUpdated();
        }, -1, null, -1, null, null);
    }
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        if (service != null) {
            ...
            if (termuxSession != null) termuxSession.getExecutionCommand().shellName = text;
        }
    }
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getTermuxSessionsSize() >= MAX_SESSIONS) {
            ...
            if (currentSession == null) { workingDirectory = mActivity.getProperties().getDefaultWorkingDirectory(); }
    ...
    public void setCurrentStoredSession() {
        TerminalSession currentSession = mActivity.getCurrentSession();
        if (currentSession != null) mActivity.getPreferences().setCurrentSession(currentSession.mHandle);
        else mActivity.getPreferences().setCurrentSession(null);
    }
    ...
    public void termuxSessionListNotifyUpdated() { mActivity.termuxSessionListNotifyUpdated(); }
    ...
    String toToastTitle(TerminalSession session) {
        ...
        if (!TextUtils.isEmpty(session.mSessionName)) { toastTitle.append(" ").append(session.mSessionName); }
    ...
    public void checkForFontAndColors() {
        try {
            ...
            if (colorsFile.isFile()) {
                try (InputStream in = new FileInputStream(colorsFile)) { props.load(in); }
            }
